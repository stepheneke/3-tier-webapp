...
echo $(aws rds describe-db-clusters --region us-east-1 --db-cluster-identifier labvpcdbcluster --query 'DBClusters[*].DBClusterMembers[?IsClusterWriter==`true`].DBInstanceIdentifier') | awk '{ print substr ($0, 6, length($0)-10) }' > list.txt
